...
inline std::shared_ptr<PhysicsObjects> asPhysicsObjects(const std::shared_ptr<NanoMuons> muons) {
  ...
  for (auto muon : *muons) { physicsObjects->push_back(muon->GetPhysicsObject()); }
...
inline std::shared_ptr<NanoMuons> asNanoMuons(
    const std::shared_ptr<Collection<std::shared_ptr<PhysicsObject>>> physicsObjects) {
  if (!physicsObjects) { return nullptr; }
  auto nanoMuons = std::make_shared<NanoMuons>();
  for (auto physicsObject : *physicsObjects) { nanoMuons->push_back(asNanoMuon(physicsObject)); }
  return nanoMuons;
}
...
inline std::shared_ptr<NanoDimuonVertices> asNanoDimuonVertices(
    const std::shared_ptr<Collection<std::shared_ptr<PhysicsObject>>> physicsObjects,
    const std::shared_ptr<Event> event) {
...
inline std::shared_ptr<NanoJets> asNanoJets(const std::shared_ptr<PhysicsObjects> physicsObjects) {
  ...
  for (auto physicsObject : *physicsObjects) { nanoJets->push_back(asNanoJet(physicsObject)); }
  return nanoJets;
}
...
inline std::shared_ptr<HepMCParticle> asHepMCParticle(const std::shared_ptr<PhysicsObject> physicsObject) {
  ...
  if (index < 0) {
    fatal() << "Error in asHepMCParticle(...). Make sure to set index of the physics object using SetIndex()."
            << std::endl;
    exit(1);
  }
