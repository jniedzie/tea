...
public:
  ...
  inline auto Get(std::string branchName, const char *file = __builtin_FILE(),
    ...
    if (valuesTypes.count(branchName) == 0 &&
        customValuesTypes.count(branchName) == 0) {
      std::string message =
          "\nTrying to access incorrect event-level branch: " + branchName;
      if (branchName.find("Weight") != std::string::npos ||
          branchName.find("Wgt") != std::string::npos ||
          branchName.find("weight") != std::string::npos ||
          branchName.find("wgt") != std::string::npos) {
        message +=
            ", it's probably fine for data if this is a gen weight branch.";
  ...
  template <typename T> T GetAs(std::string branchName) {
              ...
              try {
                ...
                try {
                  ...
                  try {
                    ...
                    error() << "Couldn't get value for branch " << branchName
                            << std::endl;
                  }
                }
              }
  ...
  inline std::shared_ptr<PhysicsObjects> GetCollection(std::string name) const {
    ...
    if (extraCollections.count(name))
      return extraCollections.at(name);
    std::string message =
        "Tried to get a collection that doesn't exist: " + name;
    throw Exception(message.c_str());
  }
  ...
  void ReplaceCollection(std::string name,
                         std::shared_ptr<PhysicsObjects> collection) {
    extraCollections[name] = collection;
  }

  const insertion_ordered_map<std::string, ExtraCollection> &
  GetExtraCollectionsDescriptions() const {
    return extraCollectionsDescriptions;
  }

  Int_t *GetIntVector(std::string branchName) {
    return valuesIntVector.at(branchName);
  }
  Bool_t *GetBoolVector(std::string branchName) {
    return valuesBoolVector.at(branchName);
  }
  Float_t *GetFloatVector(std::string branchName) {
    return valuesFloatVector.at(branchName);
  }
  Double_t *GetDoubleVector(std::string branchName) {
    return valuesDoubleVector.at(branchName);
  }
  UChar_t *GetUcharVector(std::string branchName) {
    return valuesUcharVector.at(branchName);
  }
  Char_t *GetCharVector(std::string branchName) {
    return valuesCharVector.at(branchName);
  }
  UInt_t *GetUintVector(std::string branchName) {
    return valuesUintVector.at(branchName);
  }
  UShort_t *GetUshortVector(std::string branchName) {
    return valuesUshortVector.at(branchName);
  }
  Short_t *GetShortVector(std::string branchName) {
    return valuesShortVector.at(branchName);
  }
  std::vector<unsigned int> *GetStdUintVector(std::string branchName) {
...
private:
  ...
  insertion_ordered_map<std::string, ExtraCollection>
  ...
  bool tryGet(std::shared_ptr<PhysicsObject> physicsObject,
              std::string branchName, std::pair<float, float> cuts);

  bool checkCuts(std::shared_ptr<PhysicsObject> physicsObject,
                 std::string branchName, std::pair<float, float> cuts);
};
