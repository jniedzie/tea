...
def plot_hist2D(hist, title, output_name, xbins=[], ybins=[], xaxis_title="X axis", yaxis_title="Y axis"):
  ...
  if xbins and ybins:
    ...
    if xbins[0] == 0 or ybins[0] == 0:
      new_hist = ROOT.TH2D(hist.GetName() + "_shiftedX", hist.GetTitle(), n_bins_x, xbins_d, n_bins_y, ybins_d)
...
def plot_hist2D_with_unc(
  hist, hist_unc_up, hist_unc_down, title, output_name, xbins=[], ybins=[], xaxis_title="X axis", yaxis_title="Y axis"
):
  canvas = ROOT.TCanvas("canvas", "", 800, 600)
  hist.SetTitle(title)
  hist.GetXaxis().SetTitle(xaxis_title)
...
def plot_extrapolated_histograms(extrapolation_results, edges, output_name, axis_title, title_variable):
  for i, results in enumerate(extrapolation_results):
    ...
    for category in ["nom", "up", "down"]:
      ...
      info(
        f"\t#chi2={extrapolation_func.GetChisquare():.2f}, ndf={extrapolation_func.GetNDF():.2f}, #chi2/ndf={normChi2:.2f}, p-value={p:.2f}"
      )
      hist_before.Draw("E")
...
def main():
  ...
  if config.histogram1D:
    ...
    ratio_hist, ratio_unc_hist_up, ratio_unc_hist_down = sfProducer.getDataMCRatioHists1D(
      data_histogram, background_histogram
    )
    ...
    plot_hist1D(
      background_hist, "MC Background Histogram", f"mc_hist1D{config.year}", config.correction_inputs[0]["name"]
    )
    plot_hist1D(ratio_hist, "Data / MC Scale Factor", f"sf_ratio1D{config.year}", config.correction_inputs[0]["name"])
    plot_hist1D(
      ratio_unc_hist_up,
      "Data / MC Scale Factor Uncertainty up",
      f"sf_ratio_uncertainty1D_up{config.year}",
      config.correction_inputs[0]["name"],
    )
    plot_hist1D(
      ratio_unc_hist_down,
      "Data / MC Scale Factor Uncertainty down",
      f"sf_ratio_uncertainty1D_down{config.year}",
      config.correction_inputs[0]["name"],
    )
  ...
  if config.histograms1D:
    ...
    for sfBin, histogram in config.histograms1D.items():
      ...
      plot_hist1D(
        data_hist, f"Data Histogram {sfBin}", f"data_hist_{sfBin}_{config.year}", config.correction_inputs[0]["name"]
      )
      plot_hist1D(
        background_hist,
        f"MC Background Histogram {sfBin}",
        f"mc_hist_{sfBin}_{config.year}",
        config.correction_inputs[0]["name"],
      )
      plot_hist1D(
        ratio_hist,
        f"Data / MC Scale Factor {sfBin}",
        f"sf_ratio_{sfBin}_{config.year}",
        config.correction_inputs[0]["name"],
      )
      plot_hist1D(
        ratio_unc_hist_up,
        f"Data / MC Scale Factor Uncertainty up {sfBin}",
        f"sf_ratio_uncertainty_up_{sfBin}_{config.year}",
        config.correction_inputs[0]["name"],
      )
      plot_hist1D(
        ratio_unc_hist_down,
        f"Data / MC Scale Factor Uncertainty down {sfBin}",
        f"sf_ratio_uncertainty_down_{sfBin}_{config.year}",
        config.correction_inputs[0]["name"],
      )
    ...
    for category, sfs in scale_factors.items():
      if isinstance(sfs, (list, tuple)) and len(sfs) >= 3:
        ...
        info(f"Warning: scale factors for category '{category}' do not have at least 3 elements: {sfs}")
  ...
  if config.histogram2D:
    ...
    ratio_hist, ratio_unc_hist_up, ratio_unc_hist_down = sfProducer.getDataMCRatioHists2D(
      data_histogram, background_histogram
    )
    ...
    if config.extrapolate_in_x or config.extrapolate_in_y:
      ...
      if config.extrapolate_in_x:
        fit_results = sfProducer.extrapolate_missing_scale_factors(
          scale_factors, edges, func=config.extrapolation_function, in_x=True, in_y=False
        )
        plot_extrapolated_histograms(
          fit_results,
          outer_edges,
          f"extrapolated_hists_{config.extrapolation_function}_x_{config.year}",
          y_title,
          x_title_,
        )
      if config.extrapolate_in_y:
        fit_results = sfProducer.extrapolate_missing_scale_factors(
          scale_factors, edges, func=config.extrapolation_function, in_x=False, in_y=True
        )
        plot_extrapolated_histograms(
          fit_results,
          inner_edges,
          f"extrapolated_hists_{config.extrapolation_function}_y_{config.year}",
          x_title,
          y_title_,
        )
    else:
    ...
    plot_hist2D(
      data_hist,
      "Data Histogram",
      f"sf_{config.year}/data_hist_{config.year}",
      outer_edges,
      inner_edges,
      x_title,
      y_title,
    ...
    plot_hist2D(
      ...
      f"sf_{config.year}/mc_hist_{config.year}",
      outer_edges,
      inner_edges,
      x_title,
      y_title,
    ...
    plot_hist2D(
      ratio_hist,
      "Data / MC Corrections",
      f"sf_{config.year}/sf_ratio_{config.year}",
      outer_edges,
      inner_edges,
      x_title,
      y_title,
    )
    plot_hist2D_with_unc(
      ratio_hist,
      ratio_unc_hist_up,
      ratio_unc_hist_down,
      "Data / MC Corrections",
    ...
    plot_hist2D(
      ratio_unc_hist_up,
      "Data / MC Corrections Uncertainty up",
      f"sf_{config.year}/sf_ratio_uncertainty_up_{config.year}",
      outer_edges,
      inner_edges,
      x_title,
      y_title,
    ...
    plot_hist2D(
      ratio_unc_hist_down,
      "Data / MC Corrections Uncertainty down",
      f"sf_{config.year}/sf_ratio_uncertainty_down_{config.year}",
      outer_edges,
      inner_edges,
      x_title,
      y_title,
    ...
    plot_hist2D(
      ratio_unc_hist_up,
      "Data / MC Corrections Uncertainty up relative",
      f"sf_{config.year}/sf_ratio_uncertainty_up_relative_{config.year}",
      outer_edges,
      inner_edges,
      x_title,
      y_title,
    ...
    plot_hist2D(
      ratio_unc_hist_down,
      "Data / MC Corrections Uncertainty down relative",
      f"sf_{config.year}/sf_ratio_uncertainty_down_relative_{config.year}",
      outer_edges,
      inner_edges,
      x_title,
      y_title,
    ...
    plot_hist2D_with_unc(
      ...
      final_hist_down,
      "Data / MC Corrections",
      f"sf_{config.year}/sf_ratio_final_{config.year}",
      outer_edges,
      inner_edges,
      x_title,
      y_title,
    ...
    plot_hist2D(
      final_hist_up,
      "Data / MC Corrections Up Variation",
      f"sf_{config.year}/sf_ratio_variation_up_final_{config.year}",
      outer_edges,
      inner_edges,
      x_title,
      y_title,
    ...
    plot_hist2D(
      final_hist_down,
      "Data / MC Corrections Down Variation",
      f"sf_{config.year}/sf_ratio_variation_down_final_{config.year}",
      outer_edges,
      inner_edges,
      x_title,
      y_title,
    ...
    plot_hist2D(
      final_hist_unc_up,
      "Data / MC Corrections Up Uncertainty",
      f"sf_{config.year}/sf_ratio_uncertainty_up_final_{config.year}",
      outer_edges,
      inner_edges,
      x_title,
      y_title,
    ...
    plot_hist2D(
      final_hist_unc_down,
      "Data / MC Corrections Down Uncertainty",
      f"sf_{config.year}/sf_ratio_uncertainty_down_final_{config.year}",
      outer_edges,
      inner_edges,
      x_title,
      y_title,
    ...
    plot_hist2D(
      final_hist_down_rel,
      "Data / MC Corrections Up Relative Variation",
      f"sf_{config.year}/sf_ratio_variation_up_relative_final_{config.year}",
      outer_edges,
      inner_edges,
      x_title,
      y_title,
    ...
    plot_hist2D(
      final_hist_down_rel,
      "Data / MC Corrections Down Relative Variation",
      f"sf_{config.year}/sf_ratio_variation_down_relative_final_{config.year}",
      outer_edges,
      inner_edges,
      x_title,
      y_title,
  ...
  if config.histograms2D:
    ...
    for sfBin, histogram in config.histograms2D.items():
      ...
      plot_hist2D(
        data_hist,
        f"Data Histogram {sfBin}",
        f"data_hist_{sfBin}_{config.year}",
        correction_inputs[0]["name"],
        correction_inputs[1]["name"],
      )
      plot_hist2D(
        background_hist,
        f"MC Background Histogram {sfBin}",
