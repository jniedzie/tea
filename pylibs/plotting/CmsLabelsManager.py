...
class CmsLabelsManager:
  ...
  def __drawExtraCmsText(self):
    ...
    if self.label_outside_axes and self.cmsText is not None:
      cms_width = self.__textWidth(self.cmsText, self.cmsTextFont, self.cmsTextSize)
      space_width = self.__textWidth("CMS x", self.cmsTextFont, self.cmsTextSize) - self.__textWidth(
        "CMSx", self.cmsTextFont, self.cmsTextSize
      )
  ...
  def __label_position(self):
    ...
    label_x = self.left + self.relPosX * (1 - self.left - self.right) if self.customLabelX else default_x
    label_y = 1 - self.top - self.relPosY * (1 - self.top - self.bottom) if self.customLabelY else default_y
  ...
  def drawLabels2D(self, canvas):
    ...
    cms_width = 0.0
    if self.cmsText is not None:
      cms_text_width = self.__textWidth(self.cmsText, self.cmsTextFont, self.cmsTextSize)
      space_width = self.__textWidth("CMS x", self.cmsTextFont, self.cmsTextSize) - self.__textWidth(
        "CMSx", self.cmsTextFont, self.cmsTextSize
      )
