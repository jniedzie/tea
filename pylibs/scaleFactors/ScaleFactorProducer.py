...
class ScaleFactorProducer:
  ...
  def __rebin2D(self, hist, hist_name, xmin, xmax, ymin, ymax):
    ...
    hist_name = hist_name + "_rebinned"
    new_histogram = TH2F(
      hist_name, hist_name, new_n_bins_x, array("d", new_bin_edges_x), new_n_bins_y, array("d", new_bin_edges_y)
    )
  ...
  def getDataMCRatio(
    self,
    data_yield,
    data_uncertainty_down,
    data_uncertainty_up,
    background_yield,
    background_uncertainty_down,
    background_uncertainty_up,
  ):
  ...
  def getHistsFromScaleFactors2D(self, scale_factors, bin_edges_x, bin_edges_y, z_bin):
    ...
    hist2D = ROOT.TH2F("hist2d", "Example 2D Histogram", nbins_x, x_edges, nbins_y, y_edges)
