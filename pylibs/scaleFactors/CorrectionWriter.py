...
class CorrectionWriter:
  ...
  def __make_category(self, name, data, edges):
    ...
    for i in range(len(edges)):
      content.append({"key": edges[i], "value": data[i]})
    return Category(nodetype="category", input=name, content=content)
  ...
  def __make_binning(self, name, content, edges, flow):
    return Binning(nodetype="binning", input=name, edges=edges, content=content, flow=flow)
